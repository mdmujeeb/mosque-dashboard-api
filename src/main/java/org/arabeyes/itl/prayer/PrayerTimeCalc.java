...
import org.arabeyes.itl.prayer.astro.*;
...
public class PrayerTimeCalc {
	...
	private static Logger logger = LogManager.getLogger(PrayerTimeCalc.class);
	/**
	 ...
	 * @return a Dms object containing qibla direction for the current location
	 */
	public Long getQiblaInDegrees() {
		return getQiblaInDegrees(loc);
	}
	...
	public static Dms getNorthQibla(Location loc) {
		...
		logger.debug(Utils.radToDeg(Math.atan2(num, denom)));
	 ...
	 * Calculate qibla direction.
	 * @param loc location where to calculate
	 * @return a long value containg qibla direction in degrees clockwise from north
	 */
	public static long getQiblaInDegrees(Location loc) {
		
		double latitude = loc.getDegreeLat();
		double longitude = loc.getDegreeLong();
